...
**Se la porta 80 e' gia' occupata** sull'host, nginx non parte affatto
(`bind() to 0.0.0.0:80 failed (98: Address in use)`, ripetuto all'infinito): i
container girano in `network_mode: host`, quindi non c'e' nessuna pubblicazione di
porte da rimappare. Se ne sceglie un'altra:

```sh
LANMNG_HTTP_PORT=8080 docker compose up --build -d
```

La stessa riga in un file `.env` accanto al compose la rende permanente.

**Il monitoraggio Docker** vuole il gruppo `docker` dell'host: il backend gira
come utente non-root e `/var/run/docker.sock` e' `root:docker`. Il compose da'
per buono il gid 999, il valore tipico su Debian e Ubuntu; se il tuo e' diverso
la pagina Servizi resta senza i container locali e il log lo dice. Ricavalo con
`getent group docker | cut -d: -f3` e mettilo nello stesso `.env`:

```sh
DOCKER_GID=123
```

Se l'host ha gia' un suo nginx sulla porta 80, l'altra strada e' il profilo
alternativo `deploy/docker-compose.host-nginx.yml`, che avvia il solo backend e
lascia al tuo nginx il compito di servire il frontend — vedi i commenti in quel
file. Il backend non ha porte TCP: il tuo nginx lo raggiunge sul unix socket
`/run/lanmng/backend.sock`, la cui cartella la crea `deploy/lanmng.tmpfiles.conf`
(da installare in `/etc/tmpfiles.d/`).

Quel profilo serve la dashboard **solo in HTTPS**, da un unico blocco server: su
`:443` con un nome a tua scelta (accanto agli altri vhost) e su `:81` per
indirizzo IP, per i client che quel nome non lo risolvono. L'HTTP in chiaro su
`:80` (per quel nome) e su `:81` rimanda all'HTTPS. Il certificato lo metti tu:
fra i SAN devono esserci il nome e l'indirizzo dell'host, e
`/etc/nginx/snippets/lanmng-tls.conf` deve contenere le righe `ssl_certificate`
(basta un link simbolico a uno snippet TLS che hai gia').
...
- **1.1.3** — Il backend non ascolta piu' su una porta TCP locale, che qualsiasi
  processo dell'host poteva raggiungere per falsificare l'indirizzo del client. Chi
  usa un nginx suo sull'host deve fare un passo di configurazione una volta sola;
  i dettagli sono nel [CHANGELOG](CHANGELOG.md#113---2026-09-24).
